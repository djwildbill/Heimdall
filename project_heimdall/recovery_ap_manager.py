...
def ensure_recovery_profile():
    if profile_exists(PROFILE):return
    # Generate once; key remains only in NetworkManager and this root-readable state file.
    ...
    for cmd in cmds:
        c,o,e=run(cmd)
        if c:raise RuntimeError(e or o)
    state({'ap_key':psk,'active':False,'last_error':None})
...
def start_ap():
    ...
    if c:
        state({'active':False,'last_error':e or o});raise RuntimeError(e or o)
    state({'active':True,'last_error':None})
    print('JOSH recovery AP active')

def stop_ap():
    if profile_exists(PROFILE):run(['nmcli','connection','down',PROFILE])
    state({'active':False,'last_error':None})
    print('JOSH recovery AP stopped')
...
def connect_saved(name):
    ...
    stop_ap()
    c,o,e=run(['nmcli','connection','up',name,'ifname',IFACE],timeout=45)
    if c:
        start_ap();raise RuntimeError('Connection failed; recovery AP restored')
    state({'active':False,'connected_profile':name,'last_error':None})
    print('connected')
...
def connect_new(ssid,password):
    ...
    if c:
        start_ap();raise RuntimeError('Connection failed; recovery AP restored')
    state({'active':False,'connected_profile':ssid,'last_error':None})
    print('connected')
...
def main():
    ...
    sp.add_parser('start');sp.add_parser('stop');sp.add_parser('status').add_argument('--show-key',action='store_true')

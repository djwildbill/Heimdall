...
class View(object):
    def __init__(self, config, impl, state=None):
        ...
        self._state = State(state={
            ...
            'aps': LabeledValue(color=BLACK, label='NET', value='0 (00)', position=self._layout['aps'],
                                label_font=fonts.Bold, text_font=fonts.Medium),
            ...
            'name': Text(value='Josh>', position=self._layout['name'], color=BLACK, font=fonts.Bold),
            'status': Text(value=self._voice.default(), position=self._layout['status']['pos'], color=BLACK,
                           font=self._layout['status']['font'], wrap=True,
                           max_length=self._layout['status']['max']),
            'shakes': LabeledValue(label='CAP ', value='0 (00)', color=BLACK,
                                   position=self._layout['shakes'], label_font=fonts.Bold,
                                   text_font=fonts.Medium),
            'mode': Text(value='SENT', position=self._layout['mode'], font=fonts.Bold, color=BLACK),
        })
    ...
    def is_normal(self):
        return self._state.get('face') not in (
            faces.INTENSE, faces.COOL, faces.BORED, faces.HAPPY, faces.EXCITED,
            faces.MOTIVATED, faces.DEMOTIVATED, faces.SMART, faces.SAD, faces.LONELY)
    ...
    def set_closest_peer(self, peer, num_total):
        if peer is None:
            ...
            if num_total > 1:
                name += ' of over 9000' if num_total > 9000 else ' of %d' % num_total
    ...
    def on_bored(self):
        self.set('face', faces.BORED); self.set('status', self._voice.on_bored()); self.update()
    def on_sad(self):
        self.set('face', faces.SAD); self.set('status', self._voice.on_sad()); self.update()
    def on_angry(self):
        self.set('face', faces.ANGRY); self.set('status', self._voice.on_angry()); self.update()
    def on_motivated(self, reward):
        self.set('face', faces.MOTIVATED); self.set('status', self._voice.on_motivated(reward)); self.update()
    def on_demotivated(self, reward):
        self.set('face', faces.DEMOTIVATED); self.set('status', self._voice.on_demotivated(reward)); self.update()
    def on_excited(self):
        self.set('face', faces.EXCITED); self.set('status', self._voice.on_excited()); self.update()
    def on_assoc(self, ap):
        self.set('face', faces.INTENSE); self.set('status', self._voice.on_assoc(ap)); self.update()
    def on_deauth(self, sta):
        self.set('face', faces.COOL); self.set('status', self._voice.on_deauth(sta)); self.update()
    def on_miss(self, who):
        self.set('face', faces.SAD); self.set('status', self._voice.on_miss(who)); self.update()
    def on_grateful(self):
        self.set('face', faces.GRATEFUL); self.set('status', self._voice.on_grateful()); self.update()
    def on_lonely(self):
        self.set('face', faces.LONELY); self.set('status', self._voice.on_lonely()); self.update()
    def on_handshakes(self, new_shakes):
        self.set('face', faces.HAPPY); self.set('status', self._voice.on_handshakes(new_shakes)); self.update()
    def on_unread_messages(self, count, total):
        self.set('face', faces.EXCITED); self.set('status', self._voice.on_unread_messages(count, total)); self.update(); time.sleep(5.0)
    def on_uploading(self, to):
        self.set('face', random.choice((faces.UPLOAD, faces.UPLOAD1, faces.UPLOAD2))); self.set('status', self._voice.on_uploading(to)); self.update(force=True)
    def on_rebooting(self):
        self.set('face', faces.BROKEN); self.set('status', self._voice.on_rebooting()); self.update()
    def on_custom(self, text):
        self.set('face', faces.DEBUG); self.set('status', self._voice.custom(text)); self.update()
